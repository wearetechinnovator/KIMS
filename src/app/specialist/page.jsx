...
import getSpecialistData from '@/app/lib/getSpecialist';
...
const Specialist = async ({ searchParams }) => {
    const getLangLoc = await getCurrentLangLoc()
    const baseURL = await getBaseUrl(true, true);
    const baseUrlOnlyLang = await getBaseUrl(true, false);
    const data = await getStaticPageContent("specialist");
    const pageContent = data?.data[0]?.pageContent;
    const pageMeta = data?.data[0]?.metaSection;
    const staticText = await getStaticText()


    const allSpecilist = await getSpecialistData.getAllSpecialist({
        langLoc: getLangLoc
    });
                        ...
                        <div className="row">
                            <div className="col-md-8 mb-3">
                                <div className="speciality-masterpage-card mt-3">
                                    ...
                                    <div className='row'>
                                        {
                                            allSpecilist?.map((os, index) => {
                                                return <div className="col-md-6" key={index}>
                                                    <div className="speciality-masterpage-card-content">
                                                        <a href={`${baseURL}/specialist/${os?.slug}`}>
