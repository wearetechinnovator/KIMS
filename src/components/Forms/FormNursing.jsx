...
const FormNursing = ({ title, type, subject }) => {
  const [basePath, setBasePath] = useState();
  const [staticTexts, setStaticTexts] = useState({});
  const [allLocation, setAllLocation] = useState();
  const [selectedLocation, setSelectedLocation] = useState(null);
  const [formData, setFormData] = useState({
    name: "", number: "", query: '', emailId: ''
  });
  ...
  const sendMail = async () => {
    ...
    try {
      ...
      const req = await fetch("/api/send-mail", {
        ...
        body: JSON.stringify({ data: htmlMsg, formType: "Nusring Recruitment", locationData: "trivandrum" }),
      });
      ...
      // Remove data
      setFormData({ name: "", number: "", query: '', emailId: '' });
      setSelectedLocation("");
      setLoading(false);
      return;
      ...
      <div className="row">
        ...
        <div className="col-md-12 mb-3">
          <textarea className="form-control"
            placeholder={staticTexts['Type your query']}
